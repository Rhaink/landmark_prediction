...
class LandmarkPredictor:
    ...
    def calculate_similarity_progressive_batch(self, crops_batch: np.ndarray, pca_model: Dict, 
                                             ...
                                             adaptive_sampling: bool = True) -> Tuple[np.ndarray, Dict]:
        """
        ...
        if adaptive_sampling and n_candidates > 1000:
            # Para datasets grandes (step_size=1), tomar muestra inteligente
            # Usar cada 4to candidato para cobertura uniforme del espacio de búsqueda
        ...
        for stage_idx, n_components in enumerate(component_stages):
            ...
            # EARLY STOPPING LOGIC - Más agresivo para mejores speedups
    ...
    def predict_landmark_l1(self, image_filename: str, step_size: int = 2) -> Dict:
        ...
        if step_size == 1:
            # Phase 2: Progressive refinement with adaptive sampling for high-precision searches
            scores, debug_info = self.calculate_similarity_progressive_batch(
                crops_batch, self.pca_model_l1, adaptive_sampling=True
            )
            method_used = 'progressive_pca_hybrid_adaptive'
        else:
            # Phase 1: Batch processing for lower precision (already optimized)
